...
AnalysisEpic::AnalysisEpic(TString infileName_, TString outfilePrefix_): Analysis(infileName_, outfilePrefix_) {};
...
void AnalysisEpic::Execute() {
  ...
  do {
    ...
    */
    ...
    //check beam finding
    if (!foundBeamElectron || !foundBeamIon) {
      numNoBeam++;
      continue;
    };
    ...
    if (includeOutputSet["2h"]) {
      ...
      for (unsigned int i = 0; i < kin -> vecHadrons.size(); i++) {
        ...
        for (unsigned int j = 0; j < kin -> vecHadrons.size(); j++) {
          ...
          FillHistos2h(wDihadron);
          if (writeDiSidisTree && HD -> IsActiveEvent()) DiST -> FillTree(wDihadron);
        }
      }
    } // dihadron post loop
