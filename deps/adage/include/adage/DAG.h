...
class DAG : public TObject
{
  public:
    ...
    /*   - "patch" refers to a set of edges between two layers, which can be one of two types:
     ...
     * - lambda arguments can be `Node*` or `NodePath*`, or both, or neither
    ...
    // `MultiLeafOp`: make a `multi` control node, which will alter the LeafOp to
